...
/* ================================
   NOVO LOADER USANDO JSON INDEXADO
   Substitui o uso de arquivos .txt por arquivos .json com indexação
=================================== */
...
// Carrega e prepara o arquivo JSON indexado
async function loadIndexedFile(path) {
  const res = await fetch(path);
  const data = await res.json();
  state.cacheIndexado = data;
  console.log("Arquivo indexado carregado:", path);
  return data;
}

// Faz busca por termos dentro do campo "tokens"
function doSearch(term) {
  if (!state.cacheIndexado) return;
  ...
  const resultados = state.cacheIndexado.filter(item =>
    termos.every(t => item.tokens.includes(t))
  );
...
function renderResults(resultados) {
  const el = document.getElementById("resultsStack") || document.getElementById("results");
  if (!el) return;

  el.innerHTML = "";
  if (resultados.length === 0) {
    el.innerHTML = "<p class='block-empty'>Nenhum resultado encontrado.</p>";
    return;
  }
  ...
  resultados.forEach(item => {
    ...
    div.className = "card";
    div.innerHTML = `
      <div class="body"><strong>${item.title}</strong><br><br>${item.text.replace(/\n/g, "<br>")}</div>
    `;
    el.appendChild(div);
  });
}
