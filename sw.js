...
self.addEventListener('fetch', e=>{
  ...
  e.respondWith(caches.match(req).then(res=> res || fetch(req).then(r=>{ const clone=r.clone(); caches.open(CACHE).then(c=> c.put(req, clone)); return r; }).catch(()=> caches.match('index.html'))));
});
