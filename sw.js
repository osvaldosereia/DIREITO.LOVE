...
self.addEventListener('activate', e=>{
  e.waitUntil((async()=>{ const ks=await caches.keys(); await Promise.all(ks.map(k=>k!==CACHE_NAME && caches.delete(k))); })());
  self.clients.claim();
});
self.addEventListener('fetch', e=>{
  const r=e.request; if(r.method!=='GET') return;
  const a=r.headers.get('accept')||''; const isHTML=a.includes('text/html');
  if(isHTML){ e.respondWith((async()=>{ try{ const f=await fetch(r); const c=await caches.open(CACHE_NAME); c.put(r,f.clone()); return f; }catch{ const c=await caches.open(CACHE_NAME); return (await c.match(r))||(await c.match('/index.html')); } })()); return; }
